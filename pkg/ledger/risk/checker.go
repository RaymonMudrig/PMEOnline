...
func (c *Checker) CheckInstrumentEligibility(instrumentCode string) (bool, error) {
	instrument, exists := c.ledger.GetInstrument(instrumentCode)
	if !exists {
...
func (c *Checker) CheckParticipantEligibility(participantCode string, side string) (bool, error) {
	participant, exists := c.ledger.GetParticipant(participantCode)
	if !exists {
		return false, &ValidationError{
...
func (c *Checker) GetIneligibleOrders() []int {
	var ineligibleNIDs []int

	c.ledger.ForEachOrder(func(order ledger.OrderEntity) bool {
		...
		if blocked {
			ineligibleNIDs = append(ineligibleNIDs, order.NID)
		}
		return true // Continue iteration
	})
